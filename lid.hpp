...
// inradius is the perpendicular distance from the centre of a square to the
// edge.
// It is used to determine the size of the region the LID descriptor describes
cv::Ptr<cv::FaceRecognizer> createLidFaceRecognizer(int inradius = 1,
...
public:
  ...
  Lidfaces(int inradius = 1, int numNeighbors = 8, double threshold = DBL_MAX)
      : mInradius(inradius), mNumNeighbors(numNeighbors), mThreshold(threshold),
        mCodebook(), mLabels(), mCenters() {}

  // Initializes and computes an Lidfaces model with images in src and
  // corresponding labels in labels. num_components will be kept for
  // classification.
  Lidfaces(cv::InputArrayOfArrays src, cv::InputArray labels, int inradius = 1,
           int numNeighbors = 8, double threshold = DBL_MAX)
      : mInradius(inradius), mNumNeighbors(numNeighbors), mThreshold(threshold),
        mCodebook(), mLabels(labels.getMat()), mCenters() {
    train(src, labels);
  }
